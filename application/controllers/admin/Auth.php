...
class Auth extends MY_Controller
{
	...
	public function login() {
		...
		if ($result == TRUE) {
				...
				$sekarang = date('Y-m-d');
				$jatuh_tempo = $this->db->query("SELECT * FROM xin_employee_contract WHERE to_date < '$sekarang'")->result();
				foreach ($jatuh_tempo as $a) {
					$this->db->query("UPDATE xin_employees SET is_active='0' WHERE user_id='$a->employee_id'");
					# code...
				}
