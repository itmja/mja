...
class Pkwt extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['breadcrumbs'] = "PKWT & SURAT TUGAS";
		...
		if(in_array('13',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/pkwt/get_pkwt", $data, TRUE);
	 ...
	 public function per_pkwt()
     {
		$session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		$data['id'] = $this->input->post('company_id');
		$role_resources_ids = $this->Xin_model->user_role_resource();
		$data['title'] = "Perpanjang PKWT";
		$data['all_departments'] = $this->Department_model->all_departments();
		$data['all_designations'] = $this->Designation_model->all_designations();
		$data['all_user_roles'] = $this->Roles_model->all_user_roles();
		$data['all_office_shifts'] = $this->Employees_model->all_office_shifts();
		$data['get_all_companies'] = $this->Xin_model->get_companies();
		$data['all_leave_types'] = $this->Timesheet_model->all_leave_types();
		$data['breadcrumbs'] = "Perpanjang PKWT";
		if(!in_array('13',$role_resources_ids)) {
			$data['path_url'] = 'myteam_employees';
		} else {
			$data['path_url'] = 'employees';
		...
		if(in_array('13',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/pkwt/per_pkwt", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	 ...
	 public function unit($id){
		...
		$data['breadcrumbs'] = "Dashboard Unit";
		// $data = $this->load->view("admin/employees/unit_dashboard");
		$data['subview'] = $this->load->view("admin/employees/unit_dashboard", $data, TRUE);
    ...
    public function employees_list()
	 ...
	 public function download_profile(){
		...
		$gambar=$user[0]->profile_picture;
		$de_file = base_url().'uploads/profile/';
		$p = $de_file.$gambar;
		$tbl_2 = '
		<table cellpadding="2" cellspacing="0" border="1">
			...
			<tr>
				<td>Foto Profil</td>
				<td><img src="'.$p.'" width="210" height="313"></td>
			</tr>
			<tr>
