...
class Bpjs extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['cid'] = $this->input->post('company_id');
		$data['lid'] = $this->input->post('location_id');
		$data['did'] = $this->input->post('department_id');
		$data['deid'] = $this->input->post('designation_id');
		$data['title'] = "BPJS Belum Aktif";
		...
		$data['breadcrumbs'] = "BPJS elum Aktif";
		...
		if(in_array('13',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/bpjs/bpjs_list", $data, TRUE);
     ...
     public function bpjs_aktif()
     {
		...
		if(in_array('13',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/bpjs/bpjs_aktif", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	 ...
	 public function bpjs_keluar()
     {
		...
		if(in_array('13',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/bpjs/bpjs_keluar", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	 ...
	 public function add(){
		...
		$noket = $this->input->post('noket');
		$nokes = $this->input->post('nokes');
		$jnket = $this->input->post('jnket');

		$add_nokes = $this->db->query("INSERT INTO xin_hrsale_module_attributes_values (user_id,module_attributes_id,attribute_value) VALUES ('$user_id','3','$nokes')");
		$add_noket = $this->db->query("INSERT INTO xin_hrsale_module_attributes_values (user_id,module_attributes_id,attribute_value) VALUES ('$user_id','4','$noket')");
		$add_jnket = $this->db->query("INSERT INTO xin_hrsale_module_attributes_values (user_id,module_attributes_id,attribute_value) VALUES ('$user_id','5','$jnket')");
		$terdafar = $this->db->query("INSERT INTO xin_hrsale_module_attributes_values (user_id,module_attributes_id,attribute_value) VALUES ('$user_id','6','5')");
		if ($terdafar) {
			$update = $this->db->query("UPDATE xin_employees SET bpjs='1' WHERE user_id ='$user_id'");
			redirect('admin/bpjs/');
		}else {
			redirect('admin/bpjs/');
		}
	...
	public function dialog_contact() {
	 ...
	 public function get_location_departments() {
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				$data = $this->security->xss_clean($data);
				$this->load->view("admin/pkwt/get_location_departments", $data);
			} else {
				redirect('admin/');
			}
		}
	...
	public function dialog_emp_overtime() {
	 ...
	 public function designation() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/pkwt/get_designations", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
	 ...
	 public function surat_tugas(){
        ...
        $location = $this->db->query("SELECT * FROM xin_office_location WHERE location_id='$location_id'")->result();
        foreach ($location as $l ) {
            $name_location = $l->city;
        }
        $department = $this->db->query("SELECT * FROM xin_departments WHERE department_id='$department_id'")->result();
        foreach ($department as $d ) {
            $name_department = $d->department_name;
        }
        $designation = $this->db->query("SELECT * FROM xin_designations WHERE designation_id='$designation_id'")->result();
        foreach ($designation as $dn ) {
            $name_designation = $dn->designation_name;
        }
        $bio = $this->db2->query("SELECT * FROM biodata_lo WHERE uid='$uid'")->result();
        foreach ($bio as $a ) {
            $nama_emp = $a->nama_depan.$a->nama_belakang;
            $alamat_emp = $a->alm_rumah;
            $no_emp = $a->no_hp;
        }
        ...
        $document = file_get_contents("skin/st.rtf");
        // isi dokumen dinyatakan dalam bentuk string
        $document = str_replace("#NOMOR_TUGAS", $no_s, $document);
        $document = str_replace("#UNIT", $name_company, $document);
        $document = str_replace("#ALM", $address_company, $document);
        $document = str_replace("#KOTA", $name_location, $document);
        $document = str_replace("#NAMA", $nama_emp, $document);
        $document = str_replace("#ALAMAT", $alamat_emp, $document);
        $document = str_replace("#NO_HP", $no_emp, $document);
        $document = str_replace("#KET", $name_designation, $document);
        $document = str_replace("#TANGGAL", $tgl_tugas, $document);
	 ...
	 public function get_sub_departments() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/pkwt/get_sub_departments", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
